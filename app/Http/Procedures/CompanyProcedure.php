...
class CompanyProcedure
{
    ...
    {
        if (!Auth::user()->hasPermission('companies_read')){
     ...
     * @throws \Psr\Container\NotFoundExceptionInterface
     */
    public function organizations(): JsonRpcResponse
    {
        if (!Auth::user()->hasPermission('companies_read')){
            return JsonRpcResponse::error(RpcErrors::PERMISSION_DENIED_TEXT, RpcErrors::FORBIDDEN_CODE);
        }
        ...
        $items = $this->service->paginateOrganization(
            request()->get('page', 1),
            request()->get('limit', 10),
            $filters
        );
    ...
    {
        if (!Auth::user()->hasPermission('companies_create')){
            return JsonRpcResponse::error(RpcErrors::PERMISSION_DENIED_TEXT, RpcErrors::FORBIDDEN_CODE);
        }
        $action_data = OrganizationActionData::createFromRequest($request);
        return JsonRpcResponse::success($this->service->createCompany($action_data));
    }
    ...
    {
        if (!Auth::user()->hasPermission('companies_update')){
            return JsonRpcResponse::error(RpcErrors::PERMISSION_DENIED_TEXT, RpcErrors::FORBIDDEN_CODE);
        }
    ...
    {
        if (!Auth::user()->hasPermission('companies_delete')){
            return JsonRpcResponse::error(RpcErrors::PERMISSION_DENIED_TEXT, RpcErrors::FORBIDDEN_CODE);
        }
