<x-app-layout>
    ...
    <h2>{{ __('common.filials') }} <a href="{{ route('filials.create') }}" class="btn btn-primary float-end" type="button">{{ __('form.create_filial') }}</a></h2>
    <div class="card mb-4">
        <div class="card-body">
...
{{--                        <td>{{$item->filial_number}}</td>--}}
                        ...
                        <td class="text-end">
                            <form action="{{ route('filials.destroy', $item->id) }}" method="POST" data-confirm="Are you sure that you want to delete this?">

                                <a href="{{ route('filials.show', $item->id) }}" title="show">
                                    <i class="fas fa-eye text-success"></i>
                                </a>

                                <a href="{{ route('filials.edit', $item->id) }}">
