<x-app-layout>
    ...
    <h2>{{ __('common.departments') }} <a href="{{ route('departments.create') }}" class="btn btn-primary float-end" type="button">{{ __('form.create_department') }}</a></h2>
    <div class="card mb-4">
        <div class="card-body">
...
{{--                        <td>{{$item->centre_department_number}}</td>--}}
                        ...
                        <td class="text-end">
                            <form action="{{ route('departments.destroy', $item->id) }}" method="POST" data-confirm="Are you sure that you want to delete this?">

                                <a href="{{ route('departments.show', $item->id) }}" title="show">
